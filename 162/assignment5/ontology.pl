...
%finds all parents of C and adds them to list
toisaroot(C, L) :-
    findall(X, parents(C,X), L).

%gets parent of C - base case
parents(C, X) :-
    is_a(C, X).

%gets parent of C and recurses to find grandparent of C
parents(C, X) :-
    is_a(C, I),
    parents(I, X).
...
%is_a or part_of
subconcept(C, X) :-
    part_of(C, X); is_a(C, X).

%the is-a case and recursion to check the next level
subconcept(C, X) :-
    is_a(C, D),
    subconcept(D,X).

%the part-of case and recursion to check the next level
subconcept(C, X) :-
    part_of(C, D),
    subconcept(D, X).